...
/// Declares whether a provider must be replaced by an opt-in strict test or
/// preview profile before consumer code runs.
///
/// InnoDI never inspects an arbitrary factory closure to guess side effects.
/// Mark live network, persistence, process, device, or similarly observable
/// providers explicitly with ``sideEffect``. Ordinary production construction
/// remains unchanged; only `InnoDITesting` validation consumes this metadata.
public enum DIProviderEffect: String, Equatable, Hashable, Sendable {
    case none
    case sideEffect
}
...
public struct DIProviderEffectRequirement: Equatable, Hashable, Sendable {
    ...
    public let effect: DIProviderEffect

    public init(providerName: String, effect: DIProviderEffect) {
        self.providerName = providerName
        self.effect = effect
    }
}

/// Generated by effect-marked, nonisolated container `Overrides` builders.
public protocol DIOverrideEffectValidating {
    static var requiredEffectOverrides: [DIProviderEffectRequirement] { get }
    var missingEffectOverrides: [DIProviderEffectRequirement] { get }
}

/// Main-actor counterpart used by `mainActor: true` container overrides.
@MainActor
public protocol DIMainActorOverrideEffectValidating {
    static var requiredEffectOverrides: [DIProviderEffectRequirement] { get }
    var missingEffectOverrides: [DIProviderEffectRequirement] { get }
}
...
///   - effect: Explicit operational-effect metadata. `.sideEffect` makes the
