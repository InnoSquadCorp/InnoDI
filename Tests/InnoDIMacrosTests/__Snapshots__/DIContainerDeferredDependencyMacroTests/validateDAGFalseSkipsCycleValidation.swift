...
struct AppContainer {
    ...
    // MARK: - Initialization
    init(serviceA: ServiceA? = nil, serviceB: ServiceB? = nil, _innoDITrace: DITraceContext = .disabled) {
        let _innoDITraceOwner = _InnoDITraceOwner(
            context: _innoDITrace,
            containerType: Self.self
        )
        self._innoDITraceOwner_serviceA = _innoDITraceOwner
        self._innoDITraceOwner_serviceB = _innoDITraceOwner
    ...
    init(_innoDITrace: DITraceContext = .disabled, _ _innoDIApplyOverrides: (inout Overrides) -> Void) {
        ...
        self.init(serviceA: _innoDIOverrides.serviceA, serviceB: _innoDIOverrides.serviceB, _innoDITrace: _innoDITrace)
    }
