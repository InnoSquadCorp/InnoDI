...
struct AppContainer {
    ...
    // MARK: - Initialization
    init(config: AppConfig, logger: Logger? = nil, feature: EmptyFeatureContainer? = nil, featureOverrides: ((inout EmptyFeatureContainer._InnoDIMountOverrides) -> Void)? = nil) {
        ...
        if let direct = feature {
            ...
            self._storage_sub_feature = .init(apply)
        } else {
            self._storage_sub_feature = .init()
        }
