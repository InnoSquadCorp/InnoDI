...
struct AppContainer {
    ...
    // MARK: - Initialization
    init(config: Config, viewModel: ViewModel? = nil, _innoDITrace: DITraceContext = .disabled) {
    ...
    init(config: Config, _innoDITrace: DITraceContext = .disabled, _ _innoDIApplyOverrides: (inout Overrides) -> Void) {
        ...
        self.init(config: config, viewModel: _innoDIOverrides.viewModel, _innoDITrace: _innoDITrace)
    }
