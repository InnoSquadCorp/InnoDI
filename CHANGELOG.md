...
- Phase N-3 `sub.child-overrides-missing` **warning** — fires when a same-file `@SubContainer` targets a child whose members are all `.input`. The generated `<name>Overrides` slot references `<ChildContainer>.Overrides`, which the child macro does not synthesize for input-only children.
